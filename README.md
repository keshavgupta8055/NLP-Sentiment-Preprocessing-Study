...
```
├── miniproject_clean.ipynb     # Main notebook — all experiments + tuning
├── IMDB.csv                    # Dataset (download separately, see below)
├── figs/                       # All output plots (auto-generated by notebook)
│   ├── accuracy_bar.png
│   ├── f1_bar.png
...
│   ├── sentiment_dist.png
│   └── review_length.png
├── acl_report.tex              # ACL two-column LaTeX report source
├── acl_report.pdf              # Compiled report (2-3 pages)
├── presentation.pptx           # 5-slide PowerPoint presentation
└── README.md                   # This file
```
...
3. Open `miniproject_clean.ipynb` in Jupyter or VS Code
...
| File | Description |
|------|-------------|
| `miniproject_clean.ipynb` | Full annotated notebook with all experiments and tuning |
| `acl_report.pdf` | 3-page ACL two-column format research report |
| `acl_report.tex` | LaTeX source (open in Overleaf — needs `figs/` folder) |
| `presentation.pptx` | 5-slide PowerPoint presentation |
| `figs/*.png` | All 7 high-resolution plots |

### Compiling the LaTeX Report

1. Upload `acl_report.tex` to [Overleaf](https://www.overleaf.com)
2. Create a `figs/` folder in the project
3. Upload all 7 PNG files from the `figs/` directory
4. Click **Compile**

---

## Global Constants (easy to change)

These are set at the top of the notebook and control everything:

```python
RANDOM_STATE = 42       # random seed for reproducibility
TEST_SIZE    = 0.20     # 20% test, 80% train
MAX_FEATURES = 10_000   # TF-IDF vocabulary size (default)
